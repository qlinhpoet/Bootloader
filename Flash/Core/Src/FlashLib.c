...
void Flash_RW_Protect(uint16_t SectorDetail)
{
	...
	FLASH->OPTCR |= ((SectorDetail & 0x07ff) << 16);
	FLASH->OPTCR |= 1<<1;
	while((FLASH->SR & FLASH_SR_BSY) == FLASH_SR_BSY);
	(void)Flash_OPT_Unlock();
}

void Flash_Read_OptByte(uint32_t* Opt1Value, uint32_t* Opt2Value)
{
	*Opt1Value = *((uint16_t*) 0x1fffc000);
	*Opt2Value = *((uint16_t*) 0x1fffc008);
}
